...
public class Hanoi {
    ...
    private void movePieces(int numberOfPieces, char from, char to, char util) {
        if (numberOfPieces == 1) {
            // Base case: If there is only one piece to move, move it from the source to the target
            movePiece(from, to);
            return;
        }

        // Move (numberOfPieces - 1) pieces from the source to the utility peg using the target peg
        movePieces(numberOfPieces - 1, from, util, to);

        // Move the remaining one piece from the source to the target
        movePiece(from, to);

        // Move (numberOfPieces - 1) pieces from the utility peg to the target peg using the source peg
